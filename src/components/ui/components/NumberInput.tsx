 ...
 *   hundredths  hundredths of a unit  21120   → "211.20"     step 100 (1 unit)
 *
...
const UNITS: Record<NumberUnit, UnitSpec> = {
  ...
  /**
   * A bare two-decimal quantity, encoded exactly as `cents` is: the canonical
   * integer is hundredths of the displayed number. Takeoff quantities and
   * labour hours are genuinely fractional (211.2 sq ft, 15.36 hrs) and the
   * engines emit them at 2dp, so rounding them into `count` would print a
   * different number on the sheet than the total was computed from. Default
   * step of 100 is one whole unit.
   */
...
export interface NumberInputProps extends PassThrough {
   ...
   * A ReactNode rather than a string so the figures inside it can be wrapped in
   * `.num`: a hint that is mostly digits, set in the body face, breaks the
   * "every number in the data face" rule as surely as a table cell would.
   */
