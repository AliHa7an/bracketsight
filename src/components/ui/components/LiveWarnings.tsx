...
export interface LiveWarning {
  ...
  /**
   * The concrete claim. A ReactNode, not a string, so the figures inside it
   * can be wrapped in `<span className="num">` — a warning whose numbers sit
   * outside the data face is the one place the rule matters most.
   */
  title: React.ReactNode;
  /** One concrete sentence. Never "please review carefully". */
  body: React.ReactNode;
  /**
   * The word that travels with the flag colour, because colour never carries
   * meaning on its own. Defaults to the severity's own name, so loans keeps
   * reading "Irreversible" with no change at the call site.
   *
   * Sections whose flag means something other than a one-way door MUST set it:
   * in paycheck the amber means "money you are about to leave behind" and in
   * trades it means unverified pricing. Neither is irreversible, and printing
   * that word over them would be false. Name the category concretely
   * ("Unverified pricing", "Missed deduction") — never soften it to "Note".
   */
  label?: string;
}
