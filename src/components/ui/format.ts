/**
 * Integer cents. Declared locally, not imported: `@/components/ui` has zero
 * dependencies and must not bind to any one engine. Structurally identical to
 * every engine's `Cents`, so values pass across the boundary unchanged.
 */
 ...
 *
 * The engines speak basis points everywhere (CLAUDE.md: "6.39% → 639"); this is
 * the only place they become prose. Kept here rather than in a section so the
 * five tools cannot disagree about how a rate is written.
 */
