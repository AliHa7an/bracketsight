 ...
 *
 * KNOWN-GAP GAP-023 / GAP-024 / GAP-025 / GAP-026 — the current effect of that
 * fail-closed rule: NINE prescribed notices are untranscribed, so **CA, TX, FL
 * and NY generate no contract at any job size. Only PA generates**, because it
 * is the one launch state where no provision prescribes wording. Transcribing
 * those nine is what unblocks four states. Each must be copied
 * character-for-character from its clause's `sourceUrl` — the fetch tool used in
 * the verification pass caps direct quotation at ~125 characters, which is
 * precisely why none of the text exists yet. Do not reconstruct from memory: a
 * paraphrase of prescribed wording is not a weaker clause, it is a
 * non-compliant contract.
 *
 * KNOWN-GAP GAP-029 — transcription alone is not enough for three of them.
 * `formatting` now RECORDS point size, boldface, capitalisation, placement,
 * owner signature, copies and detachability; nothing downstream can RENDER any
 * of it. CA, FL and TX stay blocked until it can.
 *
 * KNOWN-GAP GAP-054 — the string "UNVERIFIED — ATTORNEY REVIEW REQUIRED" still
 * lives inside the `text` field of DRAFTED clauses, i.e. inside the field this
 * module selects for rendering. Latent for the four states that fail closed,
 * live for Pennsylvania. See /KNOWN-GAPS.md.
 */
