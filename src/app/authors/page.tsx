 ...
 * Nothing here is estimated or rounded up.
 */
...
const VERIFICATION_ROWS: readonly { engine: string; rows: number; verified: number; corrections: number; unresolved: number }[] = [
...
const REVIEW_GATES: readonly { section: string; reviewer: string; status: string }[] = [
  { section: "Student loan repayment", reviewer: "Student-aid or tax practitioner", status: "Not engaged" },
  ...
  { section: "Property assessment", reviewer: "Licensed appraiser or appeal practitioner", status: "Not engaged" },
  { section: "Trades contracts", reviewer: "Construction attorney, per state", status: "Not engaged" },
...
export default function AuthorsPage() {
  return (
    <div className="mx-auto max-w-5xl px-4 py-12">
      <h1 className="max-w-[26ch]">Who writes and checks this</h1>
      ...
      <AnswerBox className="mt-6">
        ...
        checked <span className="num">{TOTALS.rows}</span> individual values across the five
        engines: <span className="num">{TOTALS.verified}</span> confirmed,{" "}
        <span className="num">{TOTALS.corrections}</span> corrected, and{" "}
        <span className="num">{TOTALS.unresolved}</span> left unresolved rather than filled in.
        No credentialed reviewer has signed off on any section yet.
      </AnswerBox>

      <h2 className="mt-12">Attribution</h2>
      ...
      {MAINTAINER ? (
        <>
          ...
          {MAINTAINER.profileUrl ? (
            <p className="mt-3 max-w-[68ch]">
              <a href={MAINTAINER.profileUrl} rel="me noopener" className={`${link} text-ink`}>
                {MAINTAINER.profileUrl}
              </a>
            </p>
          ) : null}
        ...
        <p className="mt-4 max-w-[68ch] text-dim">
          One person maintains the rule files, the engines and the writing, and that person is
          not yet named here. Rather than publish a byline nobody can check, this page publishes
          the process instead, in enough detail that any figure on the site can be traced back to
          the document it came from and disproved if it is wrong. Corrections reach the same
          person, at{" "}
      ...
      <h2 className="mt-12">What verification means here</h2>
      ...
      <p className="mt-4 max-w-[68ch] text-dim">
        ...
        it stops, and at least one citation carrying a URL and a{" "}
        <span className="num">lastVerified</span> date. Verification means one specific thing:
        somebody fetched that URL, read the value off the document that came back, and recorded
        the date. Not that the value looks right, and not that a secondary source agrees.
      </p>
      ...
      <p className="mt-4 max-w-[68ch] text-dim">
        ...
        <span className="num">[SECONDARY]</span> and stay open. Three states&rsquo; contract
        statutes are in that state today, because their legislature websites refuse automated
        requests.
      </p>
      ...
      <p className="mt-4 max-w-[68ch] text-dim">
        ...
        procedure number and returned a percentage table that did not exist in any document. The
        engine&rsquo;s own table was already correct; accepting the summary would have replaced a
        right answer with an invented one. Every figure since has been read from extracted
        statute or PDF text rather than from anything that summarises it.
      </p>

      <h2 className="mt-12">What the pass found</h2>

      <p className="mt-4 max-w-[68ch] text-dim">
        Each row below is one numeric constant or one encoded legal rule. &ldquo;Corrected&rdquo;
        means the source said something different from the code and the code was wrong.
        &ldquo;Unresolved&rdquo; means the source could not be reached or did not answer the
        question, and the value was left alone and recorded rather than guessed. A property
        section with <span className="num">13</span> of <span className="num">38</span> rows
        confirmed is not a section to act on without checking your own county, and the section
        says so on every page.
      </p>

      <div className="hairline-all mt-6 max-w-[68ch] rounded-atlas">
        <table className="density-instrument w-full">
          ...
          <thead>
            <tr className="hairline-b">
              <th scope="col" className="px-3 py-2 text-left font-medium">Engine</th>
              ...
              <th scope="col" className="px-3 py-2 text-right font-medium">Corrected</th>
              <th scope="col" className="px-3 py-2 text-right font-medium">Unresolved</th>
            </tr>
          </thead>
          <tbody>
            {VERIFICATION_ROWS.map((row) => (
              <tr key={row.engine} className="hairline-b">
                <th scope="row" className="px-3 py-2 text-left font-normal">{row.engine}</th>
            ...
            <tr>
              <th scope="row" className="px-3 py-2 text-left font-semibold">All engines</th>
      ...
      <h2 className="mt-12">Who has reviewed it: nobody, yet</h2>
      ...
      <p className="mt-4 max-w-[68ch] text-dim">
        ...
        that says so in one place, and it stays on the site until the statuses change — dropping
        it to look more finished would be the single most dishonest thing this site could do.
      </p>
      ...
      <h2 className="mt-12">What the code guarantees, independently of who wrote it</h2>
      ...
      <p className="mt-4 max-w-[68ch] text-dim">
        ...
        through. Money is held in integer cents and rates in basis points, because floating-point
        drift across three hundred and sixty monthly iterations produces a wrong total at the end
        of a thirty-year simulation.
      </p>

      <p className="mt-4 max-w-[68ch] text-dim">
        The engines carry <span className="num">399</span> tests between them and the
        application another <span className="num">69</span>. Where an agency has published its
        own worked examples, those examples are encoded as tests, so the engine has to reproduce
        the government&rsquo;s arithmetic before it can ship. Two tools refuse to answer rather
        than answer badly: contract generation is blocked for any state whose statutory notice
        has not been transcribed word for word, and a New Jersey assessment verdict is withheld
        when the ratio that governs the county is unavailable.
      </p>

      <h2 className="mt-12">How a correction actually happens</h2>
      ...
      <p className="mt-4 max-w-[68ch] text-dim">
        ...
        with the figure and, ideally, the document that contradicts it. The report is checked
        against the primary source, not against the reporter&rsquo;s confidence. If it holds, the
        rule file is edited, the <span className="num">lastVerified</span> date moves, every page
        that depends on the value recomputes on the next build, and the change is written into
        that section&rsquo;s changelog with the date and the source. If it does not hold, you get
        an answer saying which document was read and what it said.
      </p>

      <h2 className="mt-12">Where each section&rsquo;s own record lives</h2>
      ...
      <ul className="mt-4 max-w-[68ch] list-disc space-y-2 pl-5 text-dim">
        {SECTIONS.map((section) => (
          <li key={section.id}>
            ...
            — its own methodology, dated sources, editorial policy and changelog, reachable from
            the footer of every page in the section.
          </li>
        ))}
      </ul>
      ...
      <p className="hairline-t mt-10 max-w-[68ch] pt-6 text-step--1 text-dim">
        ...
        , and the wider account of what the site is on{" "}
        <Link href="/about" className={`${link} text-ink`}>
          about
        </Link>
        .
      </p>
